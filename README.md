...
## PROJECT
For the project, we decided to investigate how stochastic simulation can be used to simulate pandemics with certain characteristics. Instead of working with the SIR-model as differential equations, we modeled a population as (x, y)-coordinates in a box where each (x, y) coordinate corresponded to one person. The rate of infection could then be adjusted by changing the movement of each person in the population along with the distribution for how an infected person contaminates susceptible people. 

To simulate different scenarios, we implemented hospitalization, travelling between multiple populations, death/infection rates for different age groups and much more.
