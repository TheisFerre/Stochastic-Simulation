...
## Run-Test II
Testing the Run-TestII on the list ```[0.54, 0.67, 0.13, 0.89, 0.33, 0.45, 0.90, 0.01, 0.45, 0.76, 0.82, 0.24, 0.17]```, we find the run-vector [2, 2, 1, 1, 0, 0].
```python
...
We found that the p-value was varying a lot even though we set the constraint of only picking values where the expected frequencies were above 5. An example p-value was ```0.2689```, where we would reject our null-hypothesis using a 5% significance level.
